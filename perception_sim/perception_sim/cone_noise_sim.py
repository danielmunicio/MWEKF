...
class ConeNoiseSimulator(Node):
    def __init__(self):
        ...
        self.camera_cones_pub = self.create_publisher(ConeArrayWithCovariance, '/camera/cones')
        self.lidar_cones_pub = self.create_publisher(ConeArrayWithCovariance, '/lidar/cones')
    ...
    def add_gaussian_noise(self, x, y, camera):
        """Add Gaussian noise to x, y coordinates."""
        if camera: 
            x += np.random.normal(0, settings.camera_noise_std)
            y += np.random.normal(0, self.settings.camera_noise_std)
        else:
            x += np.random.normal(0, settings.lidar_noise_std)
            y += np.random.normal(0, self.settings.lidar_noise_std)
        return x, y
    ...
    def cone_callback(self, msg):
        ...
        for cone in msg.blue_cones:
            camera_x, camera_y = self.add_gaussian_noise(cone.point.x, cone.point.y)
            cone.point.x = camera_x
            cone.point.y = camera_y
            camera_msg.blue_cones.append(cone)
            lidar_x, lidar_y = self.add_gaussian_noise(cone.point.x, cone.point.y)
